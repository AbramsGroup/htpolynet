...
def trace(qty, edrs, outfile='plot.png', yunits=None, **kwargs):
    ...
    Args:
        ...
        outfile (str): name of output image file, defaults to 'plot.png'
        yunits (str): unit string to append to the y-axis label as ``f'{qty} ({yunits})'``; ``None`` leaves the label as bare ``qty``.
    ...
    for c in df.columns[1:]:
        ...
        if 'avgafter' in kwargs:
            ...
            ax.plot(df.iloc[:, 0], [avg[-1]] * df.shape[0], 'k-', alpha=0.3, label=f'{avg[-1]:0.2f}')
        else:
    ...
    plt.xlabel('time (ps)')
    if len(df.columns) > 2 or 'avgafter' in kwargs:
        plt.legend()
    plt.savefig(outfile)
    plt.close(fig)
...
def multi_trace(dfL, xnames, ynames, labels=[], xlabel='time [ps]',ylabel='', outfile='plot.png', **kwargs):
    ...
    """
    # disable debug-level logging and above since matplotlib has a lot of debug statements
    default_units = {'Temperature': 'K', 'Pressure': 'bar', 'Density': r'kg/m$^3$', 'Potential': 'kJ/mol'}
...
def diagnostics_graphs(logfiles, filename, **kwargs):
    ...
    for logfile in logfiles:
        ...
        # returns the underlying-dtype int (microseconds in modern
        # pandas, not nanoseconds), so /1e9 was silently off by 1000.
        df[logfile]['elapsed']=(df[logfile]['time']-df[logfile].iloc[0,time_idx]).dt.total_seconds()/3600.0
        df[logfile].to_csv(f'{bn}.csv',index=False,sep=' ',header=True)
    fig, ax = plt.subplots(1, 3, sharex=False, figsize=figsize)
    ax[0].grid(True)
    ax[1].grid(True)
    ax[2].grid(True)
    ax[0].set_ylim([0,1])
