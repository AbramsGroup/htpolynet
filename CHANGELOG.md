...
- New depot example `6-cyanate-ester.yaml`: bisphenol-A dicyanate ester (BADCy) liquid. Initial version is a *liquid-only build* (no cure reactions) — the BCY constituent is assembled at param-stage from a BPA bisphenol-A core plus two single-carbon `CY` cyanate end-groups (formaldimine, `[CH2:1]=[NH:2]` — drawn in the sp2 imino-formate active form so the future cure-stage C-N triazine-closing bond has a sacrificial H pre-allocated on each side). Mirrors example 2's `GMA = BPA + 2 HIE` build pattern rather than defining BADCy as a monolithic SMILES, so the build path is verified independently before adding the cyclotrimerization cure chemistry. Available as `htpolynet fetch-example 6`.
