...
``pack-example``  *(editable installs only)* pack the current directory as an example tarball for contribution
================= ========================
...
.. code-block:: console
  ...
  usage: htpolynet fetch-example [-h] {0,1,2,3,4,5,6,all}

  positional arguments:
    {0,1,2,3,4,5,6,all}  example to fetch: 0-liquid-styrene, 1-polystyrene,
                          2-polymethylstyrene, 3-bisgma-styrene-thermoset,
...
Fetching unpacks the requested example directly into the current directory.  For example to fetch the PACM-DGEBA epoxy thermoset example:
...
.. note::
   ...
   This subcommand is only available when ``htpolynet`` is installed in editable mode (``pip install -e .``).  It is intended for contributors who want to add new examples to the package.

``pack-example`` packs the current directory as a ``.tgz`` tarball and places it in the package's ``resources/example_depot``.  The current directory must contain ``README.md``, ``run.sh``, at least one ``.yaml`` or ``.yml`` config file, and a ``lib/molecules`` subdirectory.

.. code-block:: console

  $ htpolynet pack-example [-n N] [--overwrite]

  options:
    -h, --help   show this help message and exit
    -n N         desired index for this example (default: next available)
    --overwrite  overwrite any existing example with this index in the depot

The directory name is used as the example name.  If the directory name already starts with a number (e.g., ``4-pacm-dgeba-epoxy-thermoset``), that number is used as the index; otherwise the value of ``-n`` is prepended.  Omitting ``-n`` (or passing ``-1``) auto-assigns the next available index.  After packing, submit a pull request so the new example can be included in the upstream package.
